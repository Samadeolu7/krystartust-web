...
def create_id_fee_income_payment(bank,payment_date,transaction=None, user=None):
    income = get_id_fee_income()
    amount = IDFee.objects.all().first().amount

    return create_income_payment(bank=bank, income=income, description='ID Fee', amount=amount, payment_date=payment_date,transaction=transaction, user=user)

def create_registration_fee_income_payment(bank, payment_date,transaction=None, user=None):
    income = get_registration_fee_income()
    amount = RegistrationFee.objects.all().first().amount
    return create_income_payment(bank=bank, income=income, description='Registration Fee', amount=amount, payment_date=payment_date,transaction=transaction, user=transaction)

def create_loan_interest_income_payment( amount, payment_date, transaction=None, user=None):
    income = get_loan_interest_income()
    income_payment = IncomePayment.objects.create(income=income, description='Loan Interest', amount=amount, payment_date=payment_date, transaction=transaction, created_by=user)
    income_payment.save()
    return income_payment

def create_risk_premium_income_payment(bank, amount,payment_date, description, transaction=None, user=None):
...
def create_loan_registration_fee_income_payment(bank, amount,payment_date, description, transaction=None, user=None):
